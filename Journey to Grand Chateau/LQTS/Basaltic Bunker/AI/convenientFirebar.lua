...
function firebar.onTickNPC(v)
	...
	for k, f in ipairs(NPC.get(260)) do
		for _, b in ipairs(Block.get(620)) do
			if Colliders.collide(f, b) and not b.isHidden then
				b:remove()
				Effect.spawn(10, b.x, b.y)
				NPC.spawn(10, b.x, b.y, f.section)
				SFX.play(3)
			end
		end
		for _, b in ipairs(Block.get(621)) do
			if Colliders.collide(f, b) and not b.isHidden then
				b:remove()
				Effect.spawn(10, b.x, b.y)
				Block.spawn(109, b.x, b.y, f.section)
				SFX.play(3)
			end
		end
		for _, b in ipairs(Block.get(633)) do
			if Colliders.collide(f, b) and not b.isHidden then
				if b.contentID > 1000 then --not coin contained but not empty
					b:remove()
					Effect.spawn(10, b.x, b.y)
					NPC.spawn(b.contentID - 1000, b.x, b.y, f.section)
					SFX.play(3)
				elseif b.contentID == 0 then --empty
					b:remove()
					Effect.spawn(10, b.x, b.y)
					SFX.play(3)
				end
